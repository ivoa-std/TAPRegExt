...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:param name="thisprefix">
     <xsl:value-of select="/xs:schema/xs:annotation/xs:appinfo/vm:targetPrefix"/>
   </xsl:param>
   ...
   <xsl:variable name="cr"><xsl:text>
    ...
    <xsl:template match="xs:complexType|xs:simpleType" mode="attributes">
      <xsl:if test=".//xs:attribute">
        ...
        <attributes>
        <xsl:attribute namespace="" name="title">
           <xsl:value-of select="concat($thisprefix,':',@name)"/>
       ...
       <xsl:choose>
          ...
          <xsl:when test="$item &lt; 6 and 
                          (xs:simpleType/xs:restriction or 
                           (starts-with($type,$thisprefix) and 
    ...
    <xsl:template match="@type[starts-with(., $thisprefix)]" mode="type">
